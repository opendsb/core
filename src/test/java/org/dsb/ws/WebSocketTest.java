...
import org.dsb.ws.config.TestConfig;
	...
	public void shouldRetryUntilServerAvailable() throws Exception {
		...
		exec.submit(() -> {
			try {
				...
				RemoteRouterClient router = new RemoteRouterClient(localRouter, new HashSet<>(Arrays.asList("ws://localhost:8025/websockets/mainRouter")));
				router.start();
				Thread.sleep(1000);
		...
		exec.submit(() -> {
			...
			while(session == null) {
				try {
					...
					session = container.connectToServer(SomeClient.class, new URI("ws://localhost:8025/websockets/echo"));
					Thread.sleep(500);
				} catch(Exception e) {
					e.printStackTrace();
