...
class DefaultData(TypedData):
    ...
    def to_dict(self) -> dict:
        return {"data": self.data, "dataType": self.dataType, "concreteType": self.concreteType}
...
def dict2typeddata(dct: dict) -> TypedData:
    ...
    if not concrete_type:
        msg = 'Json object is not valid a TypedData. Attribute "concreteType" is required.'
        raise ValueError(msg)

    if not isinstance(concrete_type, str):
        msg = 'Json object is not valid a TypedData. Attribute "concreteType" must be a string.'
        raise TypeError(msg)
