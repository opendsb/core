...
class DefaultRouter(Router):
    ...
    def __init__(self):
        ...
        self.remote_peers: dict[str, RemotePeer] = {}  # dict[Connection_id, RemotePeer]
    ...
    def route_message(self, message: Message, remote: bool = False) -> None:  # noqa: FBT001, FBT002
    ...
    def _routing_task(self, message: Message, remote: bool) -> None:  # noqa: FBT001
        ...
        for piece in pieces:
            ...
            route_exists_and_has_subscriptions = self._route_exists(destination) and self._route_has_subscriptions(
                destination
            )
    ...
    def _execute_routing(self, destination: str, message: Message) -> None:
        ...
        if isinstance(message, CallMessage):
            logger.debug('Handling special case "CallMessage". Routing "ControlMessage" to ACK.')
            ack = None
            try:
                ack = ControlMessage(
