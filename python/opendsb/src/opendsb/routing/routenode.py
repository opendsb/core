...
class RouteNode:
    ...
    def __init__(self, topic: str):  # , router_id: str):  # Commented because RouteNode no longer needs the router id
        self.topic = topic
        self.subscribers: dict[str, Subscription] = {}  # dict[Subscription.id, Subscription]
    ...
    def accept(self, message: Message) -> bool:
        for subscription in self.subscribers.values():
            logger.debug(
                f'Passing message "{message}" in node "{self.topic}" to handler'[:500]
            ...
            try:
                ...
                logger.debug(
                    f'Exception in RouteNode "{self.topic}": Unable to process message "{message}".'[:500],
                    exc_info=True,
                )
        return True
    ...
    def subscribe(self, subscription_id: str, handler: Callable) -> Subscription:
        ...
        logger.debug(f'Topic "{self.topic}" subscribers: "{list(self.subscribers.values())}"')
        return subscription
